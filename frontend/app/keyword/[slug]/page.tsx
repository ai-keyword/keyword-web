    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-12 px-5 py-8 sm:px-8 lg:px-10">
        <header className="flex flex-col gap-6 border-b border-zinc-200 pb-8">
          <Link href="/" className="w-fit text-sm font-bold text-zinc-500 hover:text-zinc-950">
            #키워드
          </Link>
        ...
        >
          {imagePrompts.map((prompt) => (
            <PromptCard key={prompt.id} prompt={prompt} displayKeyword={keyword} />
          ))}
        </Section>
        ...
        >
          {textPrompts.map((prompt) => (
            <TextPromptCard key={prompt.id} prompt={prompt} displayKeyword={keyword} />
          ))}
        </Section>
      </div>
    </main>
